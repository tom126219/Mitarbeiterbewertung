...
const EmployeeList: React.FC = () => {
  ...
  const [newEmployee, setNewEmployee] = useState<Omit<Employee, 'id' | 'evaluationCount'>>({
    name: '',
    beruf: '',
    sparte: '',
    hireDate: new Date(),
  ...
  useEffect(() => {
    const syncEvaluationCounts = async () => {
      if (employees && Array.isArray(employees) && employees.length > 0) {
        try {
          const updatedEmployees = await Promise.all(
            employees.map(async (employee) => {
              const newCount = await employeeStore.syncEvaluationCount(employee.id);
              return { ...employee, evaluationCount: newCount };
            })
          );
          setEmployees(updatedEmployees);
        } catch (err) {
          console.error('Error syncing evaluation counts:', err);
          setError('Fehler beim Synchronisieren der Bewertungszähler');
        }
      }
    };

    // Initial sync
    syncEvaluationCounts();

    // Set up interval for subsequent syncs (every 10 seconds)
    const intervalId = setInterval(syncEvaluationCounts, 10000);
  ...
  const addEmployee = async () => {
    try {
      ...
      setEmployees(prevEmployees => Array.isArray(prevEmployees) ? [...prevEmployees, added] : [added]);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && selectedEmployee) {
      if (file.name.includes(`Bewertung ${selectedEmployee.name}`)) {
        console.log('Datei erfolgreich hochgeladen und zugeordnet:', file.name);
        try {
          const updated = await employeeStore.incrementEvaluationCount(selectedEmployee.id);
          if (updated) {
            await loadEmployees();
            const updatedEmployee = employees.find(e => e.id === selectedEmployee.id);
            setSelectedEmployee(updatedEmployee || null);
            console.log('Evaluation count incremented for employee:', selectedEmployee.name);
          } else {
            setError('Fehler beim Aktualisieren der Bewertungszahl');
          }
        } catch (err) {
        ...
        setError('Der Dateiname stimmt nicht mit dem ausgewählten Mitarbeiter überein');
      }
    } else {
      setError('Bitte wählen Sie zuerst einen Mitarbeiter aus');
    }
  };

  const openEvaluationForm = async (employeeId: number) => {
  ...
  const closeEvaluationForm = async () => {
    ...
    if (selectedEmployee) {
      try {
        ...
        if (result.success) {
          ...
          console.log('Imported evaluation:', evaluationData);
        ...
        alert('Fehler beim Importieren der Bewertung: ' + error);
      }
    } else {
    ...
    <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              ...
              <Select 
                ...
                onValueChange={(value) => setSelectedEmployee(employees.find(e => e.id.toString() === value) || null)}
              >
            ...
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <Button
                ...
                variant="outline"
                className="w-full h-full bg-green-100 text-green-600 hover:bg-green-200 border-green-200"
              >
                <Upload className="mr-2 h-4 w-4" /> Bewertung hochladen
              </Button>
            </div>
          </div>
          {error && (
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Button 
                ...
                onClick={() => openEvaluationForm(selectedEmployee.id)}
              >
                <FileText className="mr-2 h-4 w-4" /> Bewertungsfragebogen erstellen
              </Button>
              <Button 
                variant="outline" 
      ...
      <Dialog open={isEvaluationFormOpen} onOpenChange={setIsEvaluationFormOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            {/* Remove the DialogTitle completely */}
          </DialogHeader>
          <EvaluationForm
            ...
            <div className="space-y-6">
              <Card className="bg-blue-50 p-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <p><strong className="font-semibold">Datum:</strong> {new Date(lastEvaluation.date).toLocaleDateString()}</p>
